    ...
    <transition>
        <div class="tableware" v-if="isShow">
            <div class="actions-sheet">
                ...
                <h2 class="action-sheet-header">
                    <span class="tag"></span>
                    <span class="title">餐具份数</span>
                    <span class="tag"></span>
                </h2>
                <ul>
                    <li :class="{'selected':selectItem == item}" @click="clickItem(item)" v-for="(item,index) in tablevareList" :key="index">
                        {{item}}
                    </li>
                </ul>
            </div>
